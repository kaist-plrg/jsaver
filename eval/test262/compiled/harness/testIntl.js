...
function testWithIntlConstructors(f) {
  var constructors = ["Collator", "NumberFormat", "DateTimeFormat"]; // Optionally supported Intl constructors.
...
function getInvalidLanguageTags() {
  ...
  "x-12345-12345-en-US", "x-en-US-12345-12345", "x-en-u-foo", "x-en-u-foo-u-bar", "x-u-foo", // underscores in different parts of the language tag
  "de_DE", "DE_de", "cmn_Hans", "cmn-hans_cn", "es_419", "es-419-u-nu-latn-cu_bob", "i_klingon", "cmn-hans-cn-t-ca-u-ca-x_t-u", "enochian_enochian", "de-gregory_u-ca-gregory", "en\0", // null-terminator sequence
  ...
  "de-u-", "de-u-ca-", "de-u-ca-gregory-", "si-x", // incomplete private-use tags
  "x-", "x-y-"]; // make sure the data above is correct
...
function isCanonicalizedStructurallyValidLanguageTag(locale) {
  ...
  function canonicalizeLanguageTag(locale) {
    ...
    var i = 0; // handle standard part: all subtags before first variant or singleton subtag
    ...
    variants.sort(); // handle extensions
    ...
    while (i < subtags.length && subtags[i] !== "x") {
      ...
      if (extensionKey === "u") {
        var j = extensionStart + 1; // skip over leading attributes
    ...
    extensions.sort(); // handle private use
...
function testOption(Constructor, property, type, values, fallback, testOptions) {
  ...
  var testValues, options, obj, expected, actual, error; // test that the specified values are accepted. Also add values that convert to specified values.
  ...
  if (type === "string") {
    var invalidValues = ["invalidValue", -1, null]; // assume that we won't have values in caseless scripts
    ...
    numberingSystems.forEach(function (numbering) {
      ...
      function buildExpected(raw, patternParts) {
        ...
        if (period === -1) {
          ...
          return patternParts[1] + toNumbering(raw.substring(0, period)) + patternParts[2] + toNumbering(raw.substring(period + 1)) + patternParts[3];
        }
      }
