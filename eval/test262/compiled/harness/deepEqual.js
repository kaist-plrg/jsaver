...
assert.deepEqual = function (actual, expected, message) {
  ...
  assert(assert.deepEqual._compare(actual, expected), "Expected ".concat(format(actual), " to be structurally equal to ").concat(format(expected), ". ").concat(message || ''));
};
...
assert.deepEqual.format = function (value, seen) {
  ...
  switch (_typeof(value)) {
    case 'string':
      return typeof JSON !== "undefined" ? JSON.stringify(value) : "\"".concat(value, "\"");
    ...
    case 'object':
      ...
      if (value instanceof Date) return "Date \"".concat(value.toISOString(), "\"");
      ...
      if (typeof Set !== "undefined" && value instanceof Set) {
        return "Set {".concat(Array.from(value).map(function (value) {
          _newArrowCheck(this, _this);

          return assert.deepEqual.format(value, seen);
        }.bind(this)).join(', '), "}").concat(usage.used ? " as #".concat(usage.id) : '');
      }

      if (typeof Map !== "undefined" && value instanceof Map) {
        return "Map {".concat(Array.from(value).map(function (pair) {
          _newArrowCheck(this, _this);

          return "".concat(assert.deepEqual.format(pair[0], seen), " => ").concat(assert.deepEqual.format(pair[1], seen), "}");
        }.bind(this)).join(', '), "}").concat(usage.used ? " as #".concat(usage.id) : '');
      }
      ...
      return "".concat(tag ? "".concat(tag, " ") : '', "{ ").concat(Object.keys(value).map(function (key) {
        _newArrowCheck(this, _this);

        return "".concat(key.toString(), ": ").concat(assert.deepEqual.format(value[key], seen));
      }.bind(this)).join(', '), " }").concat(usage.used ? " as #".concat(usage.id) : '');
...
assert.deepEqual._compare = function () {
  ...
  function compareEquality(a, b, cache) {
    return compareIf(a, b, isOptional, compareOptionality) || compareIf(a, b, isPrimitiveEquatable, comparePrimitiveEquality) || compareIf(a, b, isObjectEquatable, compareObjectEquality, cache) || NOT_EQUAL;
  }

  function compareIf(a, b, test, compare, cache) {
    return !test(a) ? !test(b) ? UNKNOWN : NOT_EQUAL : !test(b) ? NOT_EQUAL : cacheComparison(a, b, compare, cache);
  }
  ...
  function tryCompareTypeOfEquality(a, b) {
    return _typeof(a) !== _typeof(b) ? NOT_EQUAL : UNKNOWN;
  }
  ...
  function comparePrimitiveEquality(a, b) {
    ...
    return tryCompareStrictEquality(a, b) || tryCompareTypeOfEquality(a, b) || compareIf(a, b, isNaNEquatable, compareNaNEquality) || NOT_EQUAL;
  }
  ...
  function compareObjectEquality(a, b, cache) {
    ...
    return getCache(cache, a, b) || setCache(cache, a, b, EQUAL) // consider equal for now
    || cacheComparison(a, b, tryCompareStrictEquality, cache) || cacheComparison(a, b, tryCompareToStringTagEquality, cache) || compareIf(a, b, isValueOfEquatable, compareValueOfEquality) || compareIf(a, b, isToStringEquatable, compareToStringEquality) || compareIf(a, b, isArrayLikeEquatable, compareArrayLikeEquality, cache) || compareIf(a, b, isStructurallyEquatable, compareStructuralEquality, cache) || compareIf(a, b, isIterableEquatable, compareIterableEquality, cache) || cacheComparison(a, b, fail, cache);
  }

  function isBoxed(value) {
    return value instanceof String || value instanceof Number || value instanceof Boolean || typeof Symbol === 'function' && value instanceof Symbol || typeof BigInt === 'function' && value instanceof BigInt;
  }
  ...
  function compareValueOfEquality(a, b) {
    return compareIf(a.valueOf(), b.valueOf(), isPrimitiveEquatable, comparePrimitiveEquality) || NOT_EQUAL;
  }
  ...
  function compareStructuralEquality(a, b, cache) {
    ...
    return compareIf(a, b, isIterableEquatable, compareIterableEquality, cache) || EQUAL;
  }
