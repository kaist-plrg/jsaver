...
// concrete
const CONCRETE_LOG_HOME = path.join(JSAVER_HOME, "logs", "collect", "concrete");
const CONCRETE_RESULT_HOME = path.join(RAW_HOME, "concrete");

// jsaver
const JSAVER_LOG_HOME = path.join(JSAVER_HOME, "logs", "collect", "abstract");
...
function getSoundness(babel) {
  ...
  const soundness = testDescs.map(([test, _], id) => {
  ...
  // soundness
  if (argv.soundness) fs.writeFileSync(
    `./output/soundness.csv`,
    soundness.map(data => data.join(",")).join("\n")
  );

  // plot
  if (argv.plot) fs.writeFileSync(
    "./output/plot-data.csv",
...
const argv = require('yargs')
  ...
  .option('soundness', {
    alias: 's',
    describe: 'create output/soundness.csv',
    type: 'boolean',
  })
  .option('plot', {
    alias: 'p',
    describe: 'create output/plot-data.csv',
    type: 'boolean',
  })
  .option('configurability', {
    alias: 'c',
    describe: 'create output/configurability.csv',
    type: 'boolean',
  })
  .option('apply', {
    alias: 'a',
...
if (argv.soundness || argv.plot) getSoundness();
