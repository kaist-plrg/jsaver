...
import kr.ac.kaist.jsaver.extractor.algorithm.{ TokenParser, Compiler }
import kr.ac.kaist.jsaver.spec.JsonProtocol._
import kr.ac.kaist.jsaver.spec.algorithm.token.Token
...
object CompileREPL {
  def run(version: String, secIds: Map[String, String])(
    ...
    def prompt: String = LINE_SEP + s"${CYAN}jsaver>${RESET} "
