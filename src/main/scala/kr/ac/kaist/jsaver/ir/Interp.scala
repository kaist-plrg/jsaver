...
import kr.ac.kaist.jsaver.analyzer
import kr.ac.kaist.jsaver.cfg._
import kr.ac.kaist.jsaver.error.{ InterpTimeout, NotSupported }
import kr.ac.kaist.jsaver.js.ast.{ Lexical, AST }
import kr.ac.kaist.jsaver.js.{ Parser => ESParser, _ }
...
import kr.ac.kaist.jsaver.{ TEST_MODE, LOG, DEBUG, TIMEOUT }
  ...
  final def step: Boolean = nextTarget match {
    ...
    case NextStep(cursor) => {
      ...
      catchReturn(cursor match {
        case cursor @ InstCursor(inst, rest) => interp(inst, rest)
        case NodeCursor(node) => interp(node)
      })
    ...
    inst match {
      ...
      case IApp(id, fexpr, args) => interp(fexpr) match {
        case Func(algo) => {
          ...
          val context = Context(cursorOpt, None, id, head.name, None, Some(algo), locals)
        ...
        val vOpt = (escapedBase, prop) match {
          ...
          case (ASTVal(ast), Str(name)) => ast.semantics(name) match {
            case Some((algo, asts)) => {
              ...
              val locals = getLocals(head.params, vs)
              val cursorOpt = cursorGen(body)
              val context = Context(cursorOpt, None, id, head.name, Some(ast), Some(algo), locals)
              st.ctxtStack ::= st.context
              st.context = context
